...
async def search_researchTechnologies_route(q: str = Query(..., min_length=1, description="Search term"), page: int = Query(1, ge=1, description="Page number"), limit: int = Query(None, description="Number of items per page, -1 for all")):
    return search_research_technologies(q, page, limit)

@router.get("", summary="Get a list of research technologies", tags=["Get"])
async def get_researchTechnologies_route(page: int = Query(1, ge=1, description="Page number"), limit: int = Query(None, description="Number of items per page, -1 for all")):
    return get_research_technologies(page, limit)
