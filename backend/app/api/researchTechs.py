...
async def search_researchTechnologies_route(q: str = Query(..., min_length=1, description="Search term")):
    return search_research_technologies(q)

@router.get("", summary="Get a list of research technologies", tags=["Get"])
async def get_researchTechnologies_route():
    return get_research_technologies()
