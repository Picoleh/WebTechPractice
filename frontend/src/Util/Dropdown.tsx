...
export default function Dropdown<T>({title, data, settedValueId, expandRight, onValueChange, getLabel, getId} : DropdownProps<T>) {
    ...
    return (
        <div className="relative w-full lg:w-full" ref={dropdownRef}>
            <button className="flex h-full w-full items-center justify-center gap-2 rounded border border-gray-400 px-3 py-2 text-[var(--text-color)] hover:bg-[var(--bg-color-100)] lg:w-full" onClick={() => setIsOpen(!isOpen)}>
